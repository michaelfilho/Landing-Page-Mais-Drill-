...
function calculate(product, weight) {
  ...
  if (productType === "alimentacao-natural") {
    ...
    return { quantity: Math.ceil((daily * MONTH_DAYS) / 300), detail: `${daily}g por dia` };
  }
  if (productType === "caldo-de-ossos") {
    const serving = Math.max(30, Math.round((weight * 4) / 10) * 10);
    return { quantity: Math.ceil((serving * MONTH_DAYS) / 300), detail: `${serving}ml por dia` };
  }
  const weekly = weight < 10 ? 2 : weight <= 25 ? 3 : 4;
  return { quantity: Math.ceil(weekly * (MONTH_DAYS / 7)), detail: `${weekly} unidades por semana` };
}
...
export default function ProductMiniCalculator({ product, featured = false }) {
  ...
  const result = useMemo(() => calculate(product, weight), [product, weight]);
  const full = result.quantity * product.price;
  const total = full * (1 - SUBSCRIPTIONS[cycle].discount);

  const choose = () => {
    setSubscription(cycle);
    add(product, result.quantity, { delivery: "Quantidade ideal todo mês" });
  ...
  return <div className={`${featured ? "rounded-2xl p-5" : "rounded-xl p-3"} border transition-colors duration-500`} style={{backgroundColor:product.color,color:product.ink,borderColor:product.ink,backgroundImage:"linear-gradient(135deg, rgba(255,255,255,.16), transparent 65%)",boxShadow:featured?`5px 5px 0 ${product.ink}`:"none"}}>
    ...
    <div className="mt-2 flex gap-2">
      ...
      <select value={cycle} onChange={(event) => setCycle(event.target.value)} className={`${featured?"h-11 px-3 text-sm":"h-9 px-2 text-xs"} min-w-0 flex-1 rounded-lg border border-line bg-white font-bold text-ink outline-none focus:border-leaf`} aria-label={`Plano para ${product.name}`}><option value="mensal">Mensal · 10%</option><option value="trimestral">Trimestral · 13%</option><option value="semestral">Semestral · 15%</option><option value="anual">Anual · 18%</option></select>
    </div>
    ...
    <p className={`${featured?"mt-3 text-[10px]":"mt-1.5 text-[8px]"} text-center leading-tight opacity-75`}>Sempre a quantidade ideal por mês, em qualquer plano · confirme com seu veterinário.</p>
  </div>;
}
